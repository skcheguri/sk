...
export function useListingDetail(id: string | undefined): {
  ...
  const fetchListing = useCallback(async () => {
    ...
    const { data, error: dbError } = await supabase
      .from('listings')
      .select(
        '*, owner:public.users!listings_owner_id_fkey(name, avatar_url, verified_aadhaar)'
      )
      .eq('id', id)
      .eq('status', 'active')
      .maybeSingle();
    ...
    const owner = (db.owner ?? {}) as Record<string, any>;
    ...
    const mapped: Listing = {
      ...
      price_unit: db.category === 'vacation' ? 'night' : undefined,
