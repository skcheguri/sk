  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        {([
          { key: 'all' as const, label: 'All Requests', icon: 'ri-list-check-2', color: 'text-charcoal', bg: 'bg-gray-100' },
          { key: 'pending' as const, label: 'Pending', icon: 'ri-time-line', color: 'text-gray-600', bg: 'bg-gray-100' },
          { key: 'acknowledged' as const, label: 'Acknowledged', icon: 'ri-eye-line', color: 'text-amber-600', bg: 'bg-amber-100' },
          { key: 'in_progress' as const, label: 'In Progress', icon: 'ri-loader-4-line', color: 'text-blue-600', bg: 'bg-blue-100' },
          { key: 'resolved' as const, label: 'Resolved', icon: 'ri-checkbox-circle-line', color: 'text-green-600', bg: 'bg-green-100' },
        ]).map((s) => (
          <button
            key={s.key}
            onClick={() => setFilter(s.key)}
            className={`text-left p-4 rounded-xl border transition-all cursor-pointer ${filter === s.key ? 'border-amber-500 bg-amber-50/50' : 'border-gray-100 bg-white hover:border-amber-300/50'}`}
          >
      ...
      {urgentCount > 0 && (
        <div className="bg-red-50 border border-red-100 rounded-2xl px-5 py-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-red-500 mt-0.5">High-priority issues affecting tenants&apos; daily life. Respond within 2 hours.</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex gap-2 flex-wrap">
          {(['all', 'pending', 'in_progress', 'resolved'] as FilterStatus[]).map((f) => (
            <button
              ...
              className={`text-xs font-semibold px-3 py-1.5 rounded-full border transition-all cursor-pointer whitespace-nowrap ${
                filter === f ? 'bg-amber-500 text-white border-amber-500' : 'bg-white text-gray-500 border-gray-200 hover:border-amber-300'
              }`}
            >
          ...
          <select
            ...
            className="border border-gray-200 rounded-full px-3 py-1.5 text-xs text-charcoal focus:outline-none focus:border-amber-400 cursor-pointer"
          >
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map((req) => {
            ...
            return (
              <div key={req.id} className={`bg-white rounded-2xl border p-5 transition-all ${req.urgency === 'high' && req.status !== 'resolved' ? 'border-red-100' : 'border-gray-100'}`}>
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div className={`w-11 h-11 flex items-center justify-center rounded-xl flex-shrink-0 ${req.urgency === 'high' ? 'bg-red-50' : 'bg-amber-50'}`}>
                      <i className={`${req.icon} text-xl ${req.urgency === 'high' ? 'text-red-500' : 'text-amber-600'}`} />
                    </div>
                    <div>
                ...
                {req.assignedTo && (
                  <div className="mt-3 flex items-center gap-2 bg-amber-50/50 rounded-xl px-3 py-2 border border-amber-100">
                    <div className="w-4 h-4 flex items-center justify-center">
                      <i className="ri-user-settings-line text-amber-600 text-sm" />
                    </div>
                  ...
                  {req.status !== 'resolved' && (
                    <>
                      ...
                      <div className="relative">
                        <button
                          onClick={() => setShowAssignFor(showAssignFor === req.id ? null : req.id)}
                        ...
                        {showAssignFor === req.id && (
                          <div className="absolute top-full left-0 mt-1 bg-white rounded-xl border border-gray-200 shadow-lg z-10 min-w-[200px] py-1">
                            {technicians.map((t) => (
                              <button
                                key={t}
                                onClick={() => handleAssign(req.id, t)}
                                className="w-full text-left px-4 py-2 text-xs text-charcoal hover:bg-amber-50 transition-colors cursor-pointer"
                              >
                                {t}
                              </button>
                            ))}
